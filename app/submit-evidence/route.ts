  ...
  try {
    ...
    if (file.type.startsWith("image/") && file.size > MAX_IMAGE_SIZE) {
      return NextResponse.json(
        {
          error:
            "Image too large. Max size is 3MB. Please compress using TinyPNG before uploading.",
        },
        { status: 400 }
      );
    }
    ...
    if (file.type === "application/pdf" && file.size > MAX_PDF_SIZE) {
      return NextResponse.json(
        {
          error:
            "PDF too large. Max size is 8MB. Please compress using iLovePDF or SmallPDF before uploading.",
        },
        { status: 400 }
      );
    }
