...
function getCountryDisplayName(value: string | null | undefined): string {
  ...
  const found = Object.values(COUNTRY_NAME_MAP).find(
...
function normalizeCountry(value: string | null | undefined): string {
  ...
  const foundEntry = Object.entries(COUNTRY_NAME_MAP).find(
    ([_k, name]) => name.toLowerCase() === v.toLowerCase()
  );
...
export default async function RottenIndexPage({
  ...
  const currentScopeLabel = selectedCountryCode
    ? `Companies only — ${getCountryDisplayName(
        selectedDbValue || selectedCountryCode
      )}`
    : "Companies only — All countries";

  const debugParam = (searchParams &&
    (searchParams as any).debug) as string | string[] | undefined;
  const showDebug =
    typeof debugParam === "string" && debugParam.length > 0;
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        <header className="mb-8">
          ...
          <p className="text-gray-600">
            Ranking companies by Rotten Score based on public evidence
            of harm, misconduct, and corporate behavior.
          </p>
        </header>
